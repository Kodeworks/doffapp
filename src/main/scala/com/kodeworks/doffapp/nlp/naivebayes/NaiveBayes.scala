...
/** Implements a Naive-Bayes Classifer over bags of words.
  ...
  */
@SerialVersionUID(1L)
class NaiveBayes[L: ClassTag, W](c: Iterable[Example[L, Counter[W, Double]]],
  ...
  def normScores(o: Counter[W, Double]): Map[L, Double] = {
    val (labels, _scores) = scores(o).iterator.toArray.unzip
    val scoresVector = DenseVector(_scores)
    val _normScores = numerics.exp(scoresVector - softmax(scoresVector)).toArray
    labels.zip(_normScores).toMap
  }
}

object NaiveBayes {

  class Trainer[L: ClassTag, T](wordSmoothing: Double = 0.05, classSmoothing: Double = 0.01) extends Classifier.Trainer[L, Counter[T, Double]] {
